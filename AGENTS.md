...
Разреши строку до работы, которая на это решение опирается, — и не раньше:
`adopted` без основания читается как «проверено». Разрешая, запиши в ту же
ячейку, **на чём** проверено. Отказ от базового решения оформляется новым ADR,
замещающим старый, а не молчаливым игнорированием файла.
