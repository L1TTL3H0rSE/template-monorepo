...
if (values.help) {
  console.log("node scripts/verify-template.mjs [--ref COMMIT] [--profile all|mock|identity] [--output-dir NEW_DIRECTORY]");
  process.exit(0);
}
assert.ok(["all", "mock", "identity"].includes(values.profile), "неизвестный профиль");
...
const interrupt = signal => { report.interrupted = signal; aborter.abort(); };
...
const projects = names.map(name => ({ name, steps: Object.fromEntries([
  "copy", "initialize", "consistency", "install", "build:local", "lint", "typecheck", "test", "web-build", "storybook",
...
try {
  const sourceOK = await stage(source, async () => {
    report.commit = await command("git", ["rev-parse", "--verify", `${values.ref}^{commit}`], { cwd: root });
    ...
    report.go = await run("go", ["version"]);
    return { commit: report.commit, sourceStatus };
  });
  await stage(tests, () => run(process.execPath, ["--test", "scripts/*.test.mjs"], {
  ...
  for (const project of projects) {
    ...
    for (const module of ["kit", "gotemplate"]) {
      for (const step of ["build", "vet", "test"]) await stage(steps[`go-${module}-${step}`], () => go(module, [step, "./..."], `go-${module}-${step}`), consistent && values.profile !== "identity");
    }
    ...
    }, consistent && values.profile !== "identity");
    ...
    if (values.profile === "all" && consistent && dockerOK) await live(dir, name, steps, go, log);
    else if (values.profile === "all" && consistent && !dockerOK) {
      steps.postgres.status = "blocked";
      steps.postgres.error = "Профиль требует доступного Docker; общая БД не используется.";
      save();
    }
    await stage(steps.cleanup, () => { removeOwned(workspace, dir); assert.ok(!existsSync(dir)); }, copied && !signal.aborted);
  }
} finally {
...
async function live(dir, name, steps, go, log) {
  ...
  let proc, created = false;
  try {
    const pg = await stage(steps.postgres, async () => {
      // Намеренно без compose up: только свой контейнер, tmpfs и ephemeral port.
      await run("docker", ["create", "--name", container, "--label", `template-e2e=${container}`, "--publish", "127.0.0.1::5432", "--tmpfs", "/var/lib/postgresql/data", "-e", "POSTGRES_DB=verification", "-e", "POSTGRES_USER=verification", "-e", `POSTGRES_PASSWORD=${password}`, "postgres:16-alpine"], { log: log("postgres-create") });
      created = true;
    ...
    try { await stop(proc); }
      ...
      if (created) {
        try { await command("docker", ["rm", "-f", "-v", container], { timeout: 30_000 }); }
