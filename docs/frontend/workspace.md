...
```yaml
allowBuilds:
  '@parcel/watcher': false   # осознанно не собираем
  esbuild: true              # postinstall выполняем
```

Список обязан быть полным. Начиная с pnpm 11 пакет с `postinstall`, о котором не
...
установке, требует записи здесь в том же изменении.
