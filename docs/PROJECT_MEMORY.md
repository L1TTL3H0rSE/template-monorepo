...
- **Область:** `frontend/`
- **Факт:** пакет, объявленный только у зависимости, в `node_modules`
  потребителя не появляется — импортировать его из кода нельзя, пока он не
  объявлен в `devDependencies` самого пакета. Историческая иллюстрация —
  `@storybook/vue3`: до Storybook 10 истории импортировали `Meta`/`StoryObj`
  прямо из рендерера, и его приходилось объявлять явно, хотя приходил он
  транзитивно с `@storybook/vue3-vite`. Сейчас прямого импорта нет (запрещён
  правилом `storybook/no-renderer-packages`), но само правило pnpm в силе для
...
- **Факт:** провайдер `v8` считает покрытие по инструментированному выводу.
  ...
  барель, тем «лучше» цифра. Лечит это AST-aware remapping: в vitest 3 он
  включался опцией `coverage.experimentalAstAwareRemapping: true`, в vitest 4
  опция удалена и remapping работает всегда. Проверено по отчёту: `Avatar.vue`
  и `TextField.vue`, которые тянет барель, показывают 0%.
...
### MEM-023 — мажор `vue-router` диктует nuxt, а не приложение

- **Область:** `frontend/pnpm-workspace.yaml`
- **Факт:** приложение не импортирует `vue-router` напрямую — его тянет nuxt,
  объявляя своей зависимостью, и он же безусловно дописывает в генерируемый
  `.nuxt/tsconfig.json` плагин `vue-router/volar/sfc-route-blocks`. Плагин живёт
  только в `vue-router` 5.x. Расхождение мажоров (каталог `^4.5.1` при nuxt
  4.5.2, которому нужен `^5.2.0`) даёт **две копии роутера** в дереве: nuxt
  видит свою, приложение — свою, а в бандл попадает версия из корня приложения,
  то есть nuxt исполняется на роутере старше объявленного. Симптом мягкий и
  легко пропускается: `nuxt typecheck` печатает `[Vue] Load plugin failed` и
  `plugin is not a function`, но **завершается с кодом 0**. Версия роутера
  обновляется вместе с nuxt, а не отдельно.
- **Источники:** `frontend/pnpm-workspace.yaml`,
  `frontend/applications/web/.nuxt/tsconfig.json`.
- **Проверено:** 2026-08-18.

### MEM-024 — pnpm 11 отклоняет пакеты моложе суток

- **Область:** `frontend/`
- **Факт:** pnpm 11 по умолчанию применяет supply-chain-политику
  `minimumReleaseAge` (24 часа) и падает с
  `ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION`, если в lock-файле есть запись
  свежее порога. Ловушка в порядке действий: политика проверяет **уже
  записанный lock-файл ДО пересчёта разрешений**, поэтому понизить версию в
  каталоге и переустановить — недостаточно, установка падает на старом
  lock-файле и до пересчёта не доходит. Нужен `pnpm clean --lockfile` и
  установка с нуля. Обновление зависимости в день её выхода теперь требует
  подождать сутки — либо осознанно ослабить политику.
- **Источники:** `frontend/pnpm-workspace.yaml`, `frontend/package.json`
  (`packageManager`).
- **Проверено:** 2026-08-18.

### MEM-025 — в pnpm 11 нерешённый build-скрипт роняет установку

- **Область:** `frontend/pnpm-workspace.yaml`
- **Факт:** в pnpm 10 непрошенный `postinstall` зависимости давал
  предупреждение «Ignored build scripts». В pnpm 11 это
